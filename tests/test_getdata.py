...
import os
import requests
from getwowdata.getdata import *
...
class TestGetFunctions(unittest.TestCase):
    ...
    def test_get_access_token_status_code_ok(self):
        try:
            ...
            self.fail(f"{response.status_code} status code returned from _access_token")
    
    def test__connected_realm_index_ok(self):
        try:
            response = get_connected_realm_index()
            response.raise_for_status()
        except requests.exceptions.HTTPError:
    ...
    def test_get_profession_tier_details_ok(self):
        try:
            response = get_profession_tier_details()
            response.raise_for_status()
        except requests.exceptions.HTTPError:
